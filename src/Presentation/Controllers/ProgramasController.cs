    ...
    public async Task<IActionResult> Index(string? searchTerm = null, int page = 1, int pageSize = 5)
    {
        ...
        var items = await query
            ...
            .Select(p => new ProgramaViewModel
            {
                ...
                PlanMunicipalId = p.PlanMunicipalId,
                NombrePlanMunicipal = p.PlanMunicipal != null ? p.PlanMunicipal.Nombre : "",
                SectorId = p.SectorId,
        ...
        // Estadísticas
        ViewBag.TotalProgramas = totalCount;
        ViewBag.TotalConPlanMunicipal = await _context.Programas.Where(p => !p.IsDeleted && p.PlanMunicipalId != null).CountAsync();
        ViewBag.TotalConSector = await _context.Programas.Where(p => !p.IsDeleted && p.SectorId != null).CountAsync();
